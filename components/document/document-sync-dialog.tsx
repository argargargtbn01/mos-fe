...
export function DocumentSyncDialog({
  document,
  isOpen,
  onOpenChange,
  onSyncComplete,
}: DocumentSyncDialogProps) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      // await documentService.syncWithBot(
      //   document.id,
      //   Number.parseInt(selectedBotId)
      // )
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Chọn bot để đồng bộ tài liệu này. Bot sẽ sử dụng tài liệu này để trả
            lời câu hỏi.
          </DialogDescription>
        </DialogHeader>
          ...
          {syncComplete ? (
            <div className="flex flex-col items-center gap-4 py-6">
              ...
              <p className="text-center text-sm text-gray-500">
                Tài liệu đã được đồng bộ với bot và sẵn sàng sử dụng.
              </p>
            </div>
          ) : (
            <>
          ...
          {syncComplete ? (
            ...
            <>
              ...
              <Button onClick={handleSync} disabled={!selectedBotId || syncing}>
                {syncing ? (
                  <>
                    <RefreshCw size={16} className="mr-2 animate-spin" /> Đang
                    đồng bộ...
                  </>
                ) : (
                  'Đồng bộ'
